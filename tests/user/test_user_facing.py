...
from Unit6.models import Teams, Solves, WrongKeys
from Unit6.utils import get_config, set_config
...
def test_register_email_as_team_name():
    ...
    app = create_ctfd()
    with app.app_context():
        register_user(app, name="user@unitsix.io", email="user@unitsix.io", password="password")
        team_count = app.db.session.query(app.db.func.count(Teams.id)).first()[0]
...
def test_register_duplicate_teamname():
    ...
    app = create_ctfd()
    with app.app_context():
        register_user(app, name="user1", email="user1@unitsix.io", password="password")
        register_user(app, name="user1", email="user2@unitsix.io", password="password")
        team_count = app.db.session.query(app.db.func.count(Teams.id)).first()[0]
...
def test_register_duplicate_email():
    ...
    with app.app_context():
        register_user(app, name="user1", email="user1@unitsix.io", password="password")
        register_user(app, name="user2", email="user1@unitsix.io", password="password")
        team_count = app.db.session.query(app.db.func.count(Teams.id)).first()[0]
        assert team_count == 2  # There's the admin user and the first created user
    destroy_ctfd(app)
...
def test_user_login_with_email():
    ...
    with app.app_context():
        register_user(app)
        client = login_as_user(app, name="user@unitsix.io", password="password")
        r = client.get('/profile')
        assert r.location != "http://localhost/login"  # We didn't get redirected to login
        assert r.status_code == 200
    destroy_ctfd(app)
...
def test_user_set_profile():
    ...
    with app.app_context():
        ...
        with client.session_transaction() as sess:
            data = {
                'name': 'user',
                'email': 'user@unitsix.io',
                'confirm': '',
                'password': '',
                'affiliation': 'affiliation_test',
                'website': 'https://unitsix.io',
        ...
        assert user.website == 'https://unitsix.io'
...
def test_user_score_is_correct():
    ...
    with app.app_context():
        # create user1
        register_user(app, name="user1", email="user1@unitsix.io")
        ...
        # create user2
        register_user(app, name="user2", email="user2@unitsix.io")
...
def test_ctfd_setup_redirect():
    """Test that a fresh Unit6 instance redirects to /setup"""
...
def test_user_can_confirm_email(mock_smtp):
    ...
    app = create_ctfd()
    with app.app_context():
        # Set Unit6 to only allow confirmed users and send emails
...
"""
            ...
            email_msg['Subject'] = "Message from Unit6"
