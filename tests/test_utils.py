...
from Unit6.utils import get_config, set_config, override_template, sendmail, verify_email, ctf_started, ctf_ended, export_ctf, import_ctf
from Unit6.utils import register_plugin_script, register_plugin_stylesheet
from Unit6.utils import base64encode, base64decode
from Unit6.utils import check_email_format
from Unit6.utils import update_check
...
def test_base64encode():
    ...
    if six.PY2:
        ...
        assert base64encode('user+user@unitsix.io') == 'dXNlcit1c2VyQGN0ZmQuaW8'
...
def test_base64decode():
    ...
    if six.PY2:
        ...
        assert base64decode('dXNlcit1c2VyQGN0ZmQuaW8') == 'user+user@unitsix.io'
        assert base64decode('8J-Yhg') == '😆'
...
def test_sendmail_with_mailgun_from_config_file(fake_post_request):
    ...
    with app.app_context():
        ...
        assert kwargs['data'] == {'to': ['user@user.com'], 'text': 'this is a test',
                                  'from': 'Unit6 Admin <noreply@unitsix.io>', 'subject': 'Message from Unit6'}
...
def test_export_ctf():
    """Test that Unit6 can properly export the database"""
    app = create_ctfd()
    with app.app_context():
        register_user(app)
...
def test_import_ctf():
    """Test that Unit6 can import a CTF"""
    app = create_ctfd()
...
def test_check_email_format():
    ...
    assert check_email_format('user@unitsix.io') is True
    ...
    assert check_email_format('@unitsix.io') is False
    assert check_email_format('user.io@unitsix') is False
    assert check_email_format('user\@unitsix') is False

    for invalid_email in ['user.@unitsix.io', '.user@unitsix.io', 'user@unitsix..io']:
        try:
...
def test_update_check_identifies_update(fake_get_request):
    ...
    with app.app_context():
        ...
        fake_response.json = lambda: {
            u'resource': {
                u'html_url': u'https://github.com/Unit6/Unit6/releases/tag/9.9.9',
                u'download_url': u'https://api.github.com/repos/Unit6/Unit6/zipball/9.9.9',
        ...
        update_check()
        assert get_config('version_latest') == 'https://github.com/Unit6/Unit6/releases/tag/9.9.9'
    destroy_ctfd(app)
...
def test_update_check_notifies_user():
    ...
    with app.app_context():
        app.config['UPDATE_CHECK'] = True
        set_config('version_latest', 'https://github.com/Unit6/Unit6/releases/tag/9.9.9')
        client = login_as_user(app, name="admin", password="password")
        ...
        assert 'https://github.com/Unit6/Unit6/releases/tag/9.9.9' in response
...
def test_update_check_ignores_downgrades(fake_get_request):
    ...
    with app.app_context():
        ...
        fake_response.json = lambda: {
            u'resource': {
                u'html_url': u'https://github.com/Unit6/Unit6/releases/tag/0.0.1',
                u'download_url': u'https://api.github.com/repos/Unit6/Unit6/zipball/0.0.1',
                u'published_at': u'Wed, 25 Oct 2017 19:39:42 -0000',
                u'tag': u'0.0.1',
                u'prerelease': False,
                u'id': 6,
        ...
        fake_response.json = lambda: {
            u'resource': {
                u'html_url': u'https://github.com/Unit6/Unit6/releases/tag/{}'.format(app.VERSION),
                u'download_url': u'https://api.github.com/repos/Unit6/Unit6/zipball/{}'.format(app.VERSION),
                u'published_at': u'Wed, 25 Oct 2017 19:39:42 -0000',
