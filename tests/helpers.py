...
def create_ctfd(ctf_name="Unit6", name="admin", email="admin@unitsix.io", password="password", setup=True):
    app = create_app('Unit6.config.TestingConfig')
...
def setup_ctfd(app, ctf_name="Unit6", name="admin", email="admin@unitsix.io", password="password"):
...
def register_user(app, name="user", email="user@unitsix.io", password="password"):
    with app.app_context():
        with app.test_client() as client:
...
def gen_team(db, name='name', email='user@unitsix.io', password='password'):
