...
* Fixed update checks not happening on Unit6 start. (#595)
...
* The modals to modify files, flags, tags, and hints are no longer controlled by Challenge Type Plugins and are defined in Unit6 itself.
...
* Unit6 will now check for updates against `versioning.unitsix.io`. Admins will see in the admin panel that Unit6 can be updated.
...
* Plugins can now register directories and files to be served by Unit6
    * `Unit6.plugins.register_plugin_assets_directory` registers a directory to be served
    * `Unit6.plugins.register_plugin_asset` registers a file to be served
* Plugins can now add to the admin and user menu/nav bars
    * Plugins can now add to the admin menu bar with `Unit6.plugins. register_admin_plugin_menu_bar `
    * Plugins can now add to the user menu bar with `Unit6.plugins. register_user_page_menu_bar `
...
* Some utils functions now have `Unit6.plugins` wrappers.
...
* Vagrantfile!
* Containers functionality has been moved into a [plugin](https://github.com/Unit6/Unit6-Docker)
...
* There is now an `override_template()` function allowing plugins to replace the content of any template loaded by Unit6
...
* Unit6 now has it's own Github organization!
* From a plugin you can replace most of the utils functions used by Unit6. This allows plugins to replace even more functionality within Unit6
* Unit6 now has a concept of Hints!
* You can now customize the challenge editting modals in the admin panel
* There are now links to social media pages where you can follow Unit6 to track updates.
* Unit6 now has the ability to export and import data. This lets you save your CTFs as zip files and redeploy them again and again.
...
* Challenge types
    * This means Unit6 now supports multiple kinds of challenges.
    * Challenges are now modifiable with a plugin.
* Solve types
    * This means Unit6 now supports multiple kinds of flags/keys.
