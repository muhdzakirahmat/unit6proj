from Unit6.plugins import register_plugin_assets_directory
from Unit6.plugins.keys import get_key_class
from Unit6.models import db, Solves, WrongKeys, Keys, Challenges, Files, Tags, Hints
from Unit6 import utils

import hashlib, base64


def hash_password(flag):
    salt = flag+'new'
    return hashlib.sha256(salt.encode() + flag.encode()).hexdigest()
...
class CTFdStandardChallenge(BaseChallenge):
    ...
    def create(request):
        ...
        encryptKey = hash_password(key)

        flag = Keys(chal.id, encryptKey, request.form['key_type[0]'])
    ...
    def attempt(chal, request):
        ...
        """
        encryptKey = hash_password(request.form['key'].strip())
        provided_key = encryptKey 
...
"""
Global dictionary used to hold all the Challenge Type classes used by Unit6. Insert into this dictionary to register
your Challenge Type.
"""
CHALLENGE_CLASSES = {
