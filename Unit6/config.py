...
class Config(object):
    ...
    interest of ease, Unit6 will automatically create a secret key file for you. If you wish to add this secret key to
    ...
    used to hold the Unit6 database.

    http://flask-sqlalchemy.pocoo.org/2.1/config/#configuration-keys
    '''
    SQLALCHEMY_DATABASE_URI = os.environ.get('DATABASE_URL') or 'sqlite:///{}/unitsix.db'.format(os.path.dirname(os.path.abspath(__file__)))
    ...
    '''
    SESSION_TYPE is a configuration value used for Flask-Session. It is currently unused in Unit6.
    http://pythonhosted.org/Flask-Session/#configuration
    '''
    SESSION_TYPE = "filesystem"

    '''
    SESSION_FILE_DIR is a configuration value used for Flask-Session. It is currently unused in Unit6.
    http://pythonhosted.org/Flask-Session/#configuration
    '''
    ...
    '''
    HOST specifies the hostname where the Unit6 instance will exist. It is currently unused.
    '''
    HOST = ".unitsix.io"
    ...
    '''
    MAILFROM_ADDR = "noreply@unitsix.io"

    '''
    LOG_FOLDER is the location where logs are written
    These are the logs for Unit6 key submissions, registrations, and logins
    The default location is the Unit6/logs folder
    '''
    ...
    The default destination is the Unit6/uploads folder. If you need Amazon S3 files
    you can use the Unit6 S3 plugin: https://github.com/ColdHeat/Unit6-S3-plugin
    '''
    ...
    '''
    TRUSTED_PROXIES defines a set of regular expressions used for finding a user's IP address if the Unit6 instance
    ...
    Unit6 only uses IP addresses for cursory tracking purposes. It is ill-advised to do anything complicated based
    ...
    '''
    CACHE_TYPE specifies how Unit6 should cache configuration values. If CACHE_TYPE is set to 'redis', Unit6 will make use
    of the REDIS_URL specified in environment variables. You can also choose to hardcode the REDIS_URL here.

    It is important that you specify some sort of cache as Unit6 uses it to store values received from the database.
    ...
    '''
    UPDATE_CHECK specifies whether or not Unit6 will check whether or not there is a new version of Unit6
    '''
    UPDATE_CHECK = True
