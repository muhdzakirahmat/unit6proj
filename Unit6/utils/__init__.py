...
from Unit6.models import db, Challenges, WrongKeys, Pages, Config, Tracking, Teams, Files, ip2long, long2ip
...
class CTFdSerializer(JSONSerializer):
    ...
    export the Unit6 database into a zip file.
    """
...
SERIALIZERS['unitsix'] = CTFdSerializer  # Load the custom serializer
...
def ctf_name():
    ...
    return name if name else 'Unit6'
...
def get_ip(req=None):
    """ Returns the IP address of the currently in scope request. The approach is to define a list of trusted proxies
     ...
     Unit6 does not use IP address for anything besides cursory tracking of teams and it is ill-advised to do much
...
def update_check(force=False):
    ...
    if update:
        try:
            ...
            check = requests.get(
                'https://versioning.unitsix.io/versions/latest',
                params=params,
                timeout=0.1
            ).json()
...
def export_ctf(segments=None):
    ...
    for segment in segments:
        ...
        for item in group:
            ...
            datafreeze.freeze(result, format='unitsix', fileobj=result_file)
            result_file.seek(0)
